...
func NewCoinbaseTransaction(address string) *Transaction {
	...
	//value：
	//address：
	// txOuput:=&TxOutput{10,StringToHash160(address)}
...
//交易签名
//prevTxs : 代表当前交易的输入所引用的所有OUTPUT松鼠的交易
func (tx *Transaction)Sign(privateKey ecdsa.PrivateKey,prevTxs map[string]Transaction)  {
	//处理输入，保证交易的正确性
	//检查tx中每一个输入送引用的交易哈希是否包含在prevTxsa中
	...
	for vin_id,vin:=range txCopy.Vins{
		...
		if nil!=err{
			log.Panicf("sign to transaction [%x] failed!\n",err)
		}
		//组成交易签名
...
func (tx *Transaction)Serialize() []byte {
	...
	if err:=encoder.Encode(tx); nil!=err{
		log.Panicf("serlize the tx to []byte failed %v\n",err)
	}
	return buffer.Bytes()
}
//验证签名
func (tx *Transaction) Verify(prevTxs map[string]Transaction) bool {
	...
	for vinId,vin:=range tx.Vins{
		...
		//有需要验证的数据生成的交易哈希，必须要与签名时的数据完全一致
		txCopy.TxHash=txCopy.Hash()
		//在比特币中，签名是一个数值对，r，s代表签名
		//所以要从输入夫人signature中获取
		//获取r，s，r，s长度相等
		r:=big.Int{}
		s:=big.Int{}
